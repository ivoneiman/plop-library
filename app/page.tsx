  ...
  return (
    <main className="min-h-screen bg-paper-50">
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 md:py-12 lg:px-16 lg:py-20">
        {/* Hero desktop: texto + foto (idéntico a mockup desktop de Stitch) */}
        <section className="mb-16 hidden items-center gap-8 md:flex">
          <div className="flex flex-col gap-4 md:w-1/2">
            ...
            {(config.direccion || config.horarioLunVie || config.horarioSab) && (
              <div className="z-1-shadow inline-flex flex-col items-start gap-2 self-start rounded-xl border-2 border-mustard-500/20 bg-mustard-50 p-3 text-sm font-bold text-kraft-700 sm:flex-row sm:items-center sm:gap-4">
                {config.direccion && (
        ...
        {/* Accesos rápidos desktop: tarjetas grandes con ícono, título y descripción */}
          ...
          {accesos.map((acceso) => {
            ...
            return (
              <Link
                key={acceso.href}
                href={acceso.href}
                className={`group relative flex min-h-[250px] flex-col justify-between overflow-hidden rounded-[2rem] border-2 border-kraft-100 bg-white p-6 transition-transform duration-300 hover:-translate-y-2 z-1-shadow ${clases.borde}`}
              >
                <div className={`mb-6 flex h-20 w-20 rotate-3 items-center justify-center rounded-2xl transition-transform group-hover:rotate-6 ${clases.iconoBg}`}>
          ...
          {destacados.map((item) => (
            ...
            >
              ...
              <div className="flex flex-grow flex-col justify-center p-6">
                <h3 className={`font-display mb-2 text-2xl font-black leading-tight ${item.tituloClase}`}>
                  {item.titulo}
                </h3>
